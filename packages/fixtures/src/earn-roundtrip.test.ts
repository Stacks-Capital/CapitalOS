...
describe("K10 earn round trip", () => {
  it("deposits BTC to sBTC, supplies Zest, redeems, then requests withdrawal without compressing cross-chain states", () => {
    ...
    const depositQuote = deposit.quote(ctx, {
      action: "deposit_sbtc",
    ...
    const depositPlan = deposit.buildPlan(ctx, depositQuote, {
      action: "deposit_sbtc",
      marketId: "sbtc.deposit",
      amount: "100000000",
      recipient: owner,
    ...
    assertValidPlan(depositPlan, depositQuote, {
      now: ctx.now,
      network: "mainnet",
    ...
    const supplyPlan = zest.buildPlan(ctx, supplyQuote, {
      action: "supply",
      marketId: "zest.sbtc.vault",
      amount: "99999000",
    });
    assertValidPlan(supplyPlan, supplyQuote, {
      now: ctx.now,
      network: "mainnet",
      registryVersion: ctx.registryVersion,
      sender: owner,
    ...
    assert.equal(
      zest.explainRisk(ctx, "zest.sbtc.vault").disclosures.some((line) => line.includes("not a second asset")),
      true,
    );
    assert.equal(walkConfirmed("wf_supply").state, "COMPLETED");

    const redeemQuote = zest.quote(ctx, {
      action: "withdraw_supply",
      marketId: "zest.sbtc.vault",
      amount: "99999000",
      minOut: "99999000",
    });
    const redeemPlan = zest.buildPlan(ctx, redeemQuote, {
      action: "withdraw_supply",
      marketId: "zest.sbtc.vault",
      amount: "99999000",
      minOut: "99999000",
    });
    assertValidPlan(redeemPlan, redeemQuote, {
      now: ctx.now,
      network: "mainnet",
      registryVersion: ctx.registryVersion,
      sender: owner,
    ...
    const withdrawQuote = withdraw.quote(ctx, {
    ...
    withdraw.buildPlan(ctx, withdrawQuote, {
      action: "withdraw_sbtc",
      marketId: "sbtc.withdraw",
      amount: "99999000",
      recipient,
    ...
    withdrawal = transition(withdrawal, "CONFIRMING", {
      reason: "stacks confirmed",
      actor: "worker",
      evidence: "request-id",
    });
    withdrawal = transition(withdrawal, "STEP_CONFIRMED", {
