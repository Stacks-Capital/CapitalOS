...
describe("public SDK", () => {
  ...
  it("quotes, plans and validates Zest earn without importing adapters", () => {
    ...
    assert.equal(
      os.marketsComparable({ protocol: "zest", action: "supply" }, { protocol: "granite", action: "supply" }),
      false,
    );
  ...
  it("opens Granite borrow and a Bitflow swap as unsigned plans", () => {
    ...
    assert.equal(
      swap.plan.steps[0]?.payload.kind === "stacks_contract_call" &&
        swap.plan.steps[0].payload.functionName === "swap-x-for-y-simple-range-multi",
      true,
    );
  });
  ...
  it("keeps testnet sBTC deposit and staking disabled", () => {
    ...
    assert.throws(() =>
      os.plan(quote, { action: "deposit_sbtc", marketId: "sbtc.deposit", amount: "10000", recipient: TESTNET_OWNER }),
    );
    assert.equal(executable("stake", "mainnet"), false);
    assert.equal(
      os.capabilities().some((item) => item.action === "borrow" && item.state === "disabled"),
      true,
    );
  });
});
