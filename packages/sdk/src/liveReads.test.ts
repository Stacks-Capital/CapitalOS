...
describe("loadLiveReads", () => {
  it("is mainnet-only and fail-closes Granite/Bitflow without Pyth or a pinned pool", async () => {
    await assert.rejects(() => loadLiveReads({ network: "testnet", fetch: fetchImpl }), (error: unknown) => {
    ...
    assert.throws(() => os.quote({ action: "borrow", marketId: "granite.sbtc.isolated", amount: "1000000" }), (error: unknown) => {
      return typeof error === "object" && error !== null && "code" in error && error.code === "ORACLE_STALE";
    });
    assert.throws(() => os.quote({ action: "swap", marketId: "bitflow.sbtc-usdcx", amount: "100000000" }), (error: unknown) => {
      return typeof error === "object" && error !== null && "code" in error && error.code === "ORACLE_STALE";
    });
  });
});
