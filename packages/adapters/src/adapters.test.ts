...
import { createBitflowSwapAdapter, createGraniteCreditAdapter, createSbtcDepositAdapter, createSbtcWithdrawAdapter, createZestEarnAdapter } from "./index.ts";
...
const reads: AdapterReads = {
  ...
  oracle: {
    sbtc: { price: "10000000000000", scale: "8", observedAt: "2026-09-15T12:00:00.000Z", source: "fixture", stale: false, maxAgeMs: 180_000 },
    usdcx: { price: "100000000", scale: "8", observedAt: "2026-09-15T12:00:00.000Z", source: "fixture", stale: false, maxAgeMs: 180_000 },
  },
  swap: {
...
describe("sBTC and Zest adapters", () => {
  it("quotes a mainnet deposit and requires a Bitcoin payload, not a Stacks call", () => {
    ...
    const quote = deposit.quote(mainnet, { action: "deposit_sbtc", marketId: "sbtc.deposit", amount: "10000", recipient: MAINNET_OWNER, maxFee: "200" });
    assert.equal(quote.executable, true);
    assert.equal(quote.expectedOutput[0]?.quantity, 9800n);
    const plan = deposit.buildPlan(mainnet, quote, { action: "deposit_sbtc", marketId: "sbtc.deposit", amount: "10000", recipient: MAINNET_OWNER, maxFee: "200" });
    assert.equal(plan.steps[0]?.payload.kind, "bitcoin_deposit");
    assert.equal(validatePlan(plan, quote, { now: mainnet.now, network: "mainnet", registryVersion: mainnet.registryVersion, sender: "SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR" }).ok, true);
  });

  it("refuses a testnet deposit plan because the capability is disabled", () => {
    const testnet = ctx("testnet");
    const deposit = createSbtcDepositAdapter(reads);
    const quote = deposit.quote(testnet, { action: "deposit_sbtc", marketId: "sbtc.deposit", amount: "10000", recipient: TESTNET_OWNER });
    assert.equal(quote.executable, false);
    assert.throws(() => deposit.buildPlan(testnet, quote, { action: "deposit_sbtc", marketId: "sbtc.deposit", amount: "10000", recipient: TESTNET_OWNER }));
  });
  ...
  it("builds a deny-mode initiate-withdrawal-request plan and will not call it complete", () => {
    ...
    const quote = withdraw.quote(mainnet, { action: "withdraw_sbtc", marketId: "sbtc.withdraw", amount: "10000", recipient, maxFee: "1000" });
    const plan = withdraw.buildPlan(mainnet, quote, { action: "withdraw_sbtc", marketId: "sbtc.withdraw", amount: "10000", recipient, maxFee: "1000" });
  ...
  it("supplies and redeems sBTC through the Zest vault with min-out rounding down", () => {
    ...
    const supplyPlan = zest.buildPlan(mainnet, supply, { action: "supply", marketId: "zest.sbtc.vault", amount: "100000000" });
    ...
    const redeem = zest.quote(mainnet, { action: "withdraw_supply", marketId: "zest.sbtc.vault", amount: "100000000", minOut: "100000000" });
    zest.buildPlan(mainnet, redeem, { action: "withdraw_supply", marketId: "zest.sbtc.vault", amount: "100000000", minOut: "100000000" });
  ...
  it("quotes Granite isolated collateral and USDCx borrow with deny-mode price-feeds none", () => {
    ...
    const borrow = granite.quote(mainnet, { action: "borrow", marketId: "granite.sbtc.isolated", amount: "50000000000" });
    const plan = granite.buildPlan(mainnet, borrow, { action: "borrow", marketId: "granite.sbtc.isolated", amount: "50000000000" });
